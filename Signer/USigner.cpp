...
USigner::USigner(const char * certificate_path, const char * private_key_path) {
    ...
    this->certificate_ = PEM_read_bio_X509(cert, nullptr, nullptr, nullptr);
    BIO_free(cert);
    ...
    this->private_key_ = PEM_read_bio_PrivateKey(key, nullptr, nullptr, nullptr);
    BIO_free(key);

    this->digest_algorithm_ = const_cast<EVP_MD*>(EVP_sha256());
}
...
USigner::~USigner() {
    X509_free(this->certificate_);
    EVP_PKEY_free(this->private_key_);
}
